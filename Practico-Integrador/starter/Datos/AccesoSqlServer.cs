...
public class AccesoSqlServer : IAccesoDatos
{
    ...
    "Server=localhost;Database=practico;User Id=sa;Password=Curso.NET2026;TrustServerCertificate=True";
    ...
    public void CrearEstructura()
    {
        ...
        "Server=localhost;Database=master;User Id=sa;Password=Curso.NET2026;TrustServerCertificate=True";

        using (var connBD = new SqlConnection(cadenaAdmin))
        {
            connBD.Open();
            var cmdBD = connBD.CreateCommand();

            // CORREGIDO: Consulta nativa y segura para verificar bases de datos en SQL Server
            cmdBD.CommandText = """
            SELECT COUNT(*) 
            FROM sys.databases 
            WHERE name = 'practico';
        ...
        // CORREGIDO: Se cambia MySqlConnection por SqlConnection
    ...
    public void InsertarDatosPrueba()
    {
        ...
        {
            // --- CATEGORÍAS ---
            // CORREGIDO: Se usa SCOPE_IDENTITY() en lugar de LAST_INSERT_ID()
            string sqlCategoria = "INSERT INTO Categorias(nombre) VALUES(@nombreCat); SELECT SCOPE_IDENTITY();";
            ...
            // --- PRODUCTOS ---
            string sqlProducto = "INSERT INTO Productos (nombre, precio, stock, categoria_id) VALUES (@nombreProd, @precio, @stock, @categoriaId); SELECT SCOPE_IDENTITY();";
            ...
            // --- CLIENTES ---
            string sqlCliente = "INSERT INTO Clientes(nombre, email) VALUES(@nombreCliente, @email); SELECT SCOPE_IDENTITY();";
            ...
            // --- PEDIDOS ---
            string sqlPedido = "INSERT INTO Pedidos(cliente_id, fecha) VALUES(@clienteId, @fecha); SELECT SCOPE_IDENTITY();";
            ...
            // --- DETALLES DE PEDIDO ---
            cmd.CommandText = "INSERT INTO Detalle_pedido(pedido_id, producto_id, cantidad, precio_unitario) VALUES(@pedidoId, @productoId, @cantidad, @precioUnitario);";
    ...
    public void DemostrarRollback()
    {
        ...
        var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT precio FROM Productos WHERE id = 1";
        ...
        try
        {
            cmd.CommandText = "UPDATE Productos SET precio = precio + 1000 WHERE id = 1";
            cmd.ExecuteNonQuery();
